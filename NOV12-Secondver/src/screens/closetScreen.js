    ...
    useCallback(() => {
      const fetchItems = async () => {
        if (!userData || !userData.uid) {
          console.warn("⚠️ userData missing, skipping fetchItems");
          return;
        }
        if (!sectionId) return;
    ...
    Alert.alert("Delete Item", "Are you sure you want to delete this item?", [
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          ...
          try {
            await deleteDoc(
              doc(db, "users", userData.uid, "sections", sectionId, "items", id)
            );
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.grid}>
        ...
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate("AddItem", { sectionId, userData })}
        >
          <Image
...
const styles = StyleSheet.create({
  ...
  addContainer: { width: "45%", aspectRatio: 1, justifyContent: "center", alignItems: "center", marginTop: 10 },
  addIcon: { width: 50, height: 50 },
  addTextContainer: { alignItems: "center" },
  addText: { marginTop: 3, fontWeight: "bold", color: "black", fontSize: 16, textAlign: "center", lineHeight: 15 },
});
